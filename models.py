    ...
    class NaiveBayes_Scratch:
        ...
        def fit(self, X, y):
            ...
            for c in self.classes:
                ...
                # Calculate the Parameters for the Categorical & Gaussain Numerical
                if self.model_type == 'gaussian':
                    self.parameters[c] = {
                        "mean": X_c.mean(axis = 0),
                        "var" : X_c.var(axis = 0) + 1e-9 # Added small epilson to avoid division by zero
                    }
                else:
                    self.parameters[c] = {
        ...
        def predict(self,X):
            # Apply the Naive Bayes classification rule => P(y|X) = P(y) * P(X|y)
        ...
        def _get_likelihood(self, class_idx, x):
            if self.model_type == 'gaussian':
                ...
                probs = self.parameters[class_idx]["probs"]
